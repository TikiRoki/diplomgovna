...
function dateStrToDateInt(dateStr) {
  // Разбиваем строку на компоненты даты
  const [year, month, day] = dateStr.split("-");

  // Преобразуем компоненты в число формата YYYYMMDD
  const dateInt = parseInt(year + month + day, 10);

  return dateInt;
}

async function loadPartialData(filePath, limit = 200000) {
      ...
      .on("data", (row) => {
        if (count < limit) {
          data.push({
            ...
            object_type: parseFloat(row.building_type),
...
export default async function handler(req, res) {
  if (req.method === "GET") {
    if (
      ...
      const data = await loadPartialData("./static/data.csv");
